	...
	<view class="absolute">
		...
		<scroll-view scroll-y="true" class="scrollY" v-if="checkNum == 1">
			...
			<view class="reportBox" v-if="detailInfo.status == 3 ||detailInfo.status == 5">
				...
				<view class="">
					<DomVideoPlayer ref="domVideoPlayer" style="height: 400rpx;" :src="contentDate.videoFileList[0].url"
						loop controls muted v-if="contentDate.videoFileList && contentDate.videoFileList.length">
			...
			<view class="reportBox" v-if="(detailInfo.status == 2||detailInfo.status == 4) && isProcess">
				...
				<textarea class="textareaBox" style="margin-top: 10rpx;color: #333;" name="" id="" placeholder="请输入处理结果"
				...
				<view class="reportLine">
					...
					<view class="reportLine_R" style="display: flex;" @click="nowselectTime">
						{{nowTime | futtilvalue1}}
						<!-- 	请选择
						<uni-icons type="right" size="16" color="#999999"></uni-icons> -->
						<u-icon name="clock" style="margin-left: 10rpx;"></u-icon>
					</view>
			...
			<view class="reportBox">
				<view class="reportLine">
					...
					<view class="reportLine_R">
						{{detailInfo.projectName?detailInfo.projectName:'--'}}
				...
				<view class="reportLine">
					...
					<view class="reportLine_R">
						{{detailInfo.partName?detailInfo.partName:'--'}}
					</view>
				</view>
				<view class="reportLine">
					<view class="reportLine_L">
					...
					<view class="reportLine_R">
						{{detailInfo.facilityName?detailInfo.facilityName:'--'}}
					</view>
				</view>
				<view class="reportLine" style="margin-bottom: 0rpx;">
					<view class="reportLine_L">
						<text style="color: red;">*</text>
						{{maintenanceRecordTitle}}原因
					</view>
					<view class="reportLine_R">

					</view>
				</view>
				<textarea disabled class="textareaBox" name="" id="" placeholder="请输入维修原因"
					style="color: #333;margin-bottom: 16rpx;" v-model="detailInfo.maintenanceReason"></textarea>
			...
			<view class="blueBtn" @click="blueBtn" v-if="detailInfo.status == '1'&& receiveId">
				接收
			</view>
			<view class="blueBtn" @click="receive"
				v-if="(detailInfo.status == '2' || detailInfo.status == '4') && !isProcess &&receiveId">
				上报处理结果
			</view>
			...
			<view class="disBtns" v-if="detailInfo.status == '3' && !isProcess&&receiveId">
		...
		<u-datetime-picker :show="sptTimeshow" :minDate="minDate" v-model="contentDate.actionTime" mode="datetime"
			@confirm="sptconfirmTime" closeOnClickOverlay @close="sptcloseTime"></u-datetime-picker>
			<u-datetime-picker :show="nowsptTimeshow" :minDate="minDate" v-model="nowTime" mode="datetime"
				@confirm="nowsptconfirmTime" closeOnClickOverlay @close="nowsptcloseTime"></u-datetime-picker>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			this.querygetOrderProcessList(e.id)
		},
		methods: {
			deleteImg(idx) {
			...
			querygetOrderProcessList(e) {
				getMTProcessList(`?matterId=${e}`).then(e => {
			...
			submit() {
				...
				let a = {
					...
					// actionTime:this.nowTime,
					finishTime: dayjs(this.nowTime).format('YYYY-MM-DD HH:mm:ss'),
					actionTime:dayjs(this.nowTime).format('YYYY-MM-DD HH:mm:ss'),
			...
			nowselectTime() {
				this.nowsptTimeshow = true
			},
			nowsptcloseTime(){
				this.nowsptTimeshow = false
			},
			nowsptconfirmTime(e){
				this.nowsptTimeshow = false
				console.log(dayjs(e.value).format('YYYY-MM-DD HH:mm'))
			...
			querygetOrderDetail(e) {
				getRecordDetail(e).then(res => {
					console.log(res)
					this.detailInfo = res.data.data
					this.maintenanceRecordTitle = res.data.data.maintenanceType == '1' ? '维修' : '养护'
					this.maintenanceRecord = res.data.data.maintenanceType == '1' ? '维修处理' : '养护处理'
					this.textContent = res.data.data.maintenanceType == '1' ? '维修详情' : '养护详情'
					this.receiveId = userIdStrCheck(this.detailInfo.actionUserId, uni.getStorageSync('getInfo')
						.user.userId) //判断接收人id是否是当前登录人
				})
			},
			checkNumBtn(e) {
				console.log(e)
