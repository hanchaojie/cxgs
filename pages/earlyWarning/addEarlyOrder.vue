<template>
	<view class="absolute">
		...
		<view class="reportBox">
			...
			<textarea class="textareaBox" name="" id="" placeholder="请输入工单描述" v-model="infoDetail.context"></textarea>
		</view>
		<view class="reportBox">
			<view class="reportLine">
				<view class="reportLine_L">
		...
		<u-datetime-picker :show="sptTimeshow" v-model="finishTime" :minDate="minDate" mode="date"
		...
		methods: {
			...
			getDetail(e) {
				getwarnSupervisionId(e).then(res => {
					...
					getUserByWawo(`?wawoCode=${res.data.data.wawoCode}`).then(Response => {
			...
			gotoPatrolDetail() {
				...
				let a = {
					remark: this.infoDetail.context,
					...
					projectId: this.infoDetail.wawoCode,
