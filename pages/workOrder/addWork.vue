	...
	<view class="absolute">
		...
		<view class="reportBox">
			...
			<view class="reportLine">
				...
				<view class="reportLine_R" @click="openDictOrderType">
					{{orderTypeDictName}}
					<uni-icons type="right" size="16" color="#999999"></uni-icons>
					<!-- 	请选择
					<uni-icons type="right" size="16" color="#999999"></uni-icons> -->
			...
			<textarea class="textareaBox" name="" id="" placeholder="请输入工单描述" v-model="remark"></textarea>
		</view>
		<view class="reportBox">
			<view class="reportLine">
				<view class="reportLine_L">
		...
		<u-picker :closeOnClickOverlay="true" title='选择厂站' :show="addressPopup" ref="uPicker" :columns="columns"
		...
		<u-picker :show="order_typeDictShow" closeOnClickOverlay :columns="order_typeDictColumns"
			@confirm="OrderTyoeConfirm" @close="closeOrderTyoe" @cancel="closeOrderTyoe"></u-picker>
	...
	export default {
		...
		methods: {
			...
			getDict() {
				...
				queryDict('?dictType=order_type ').then(res => {
					this.order_typeDict = res.data.rows.map(e => {
						return {
							value: e.dictValue,
							label: e.dictLabel
						}
					})
					this.order_typeDictColumns = [res.data.rows.map(e => {
						return e.dictLabel
					})]
				})
			},
			...
			OrderTyoeConfirm(e) {
				this.orderTypeDictName = e.value[0]
				this.order_typeDictShow = false
			},
			openDictOrderType() {
				this.order_typeDictShow = true
			},
			selectpenpel() {
			...
			confirm(e) {
				...
				let wawoCode = this.findWawoCode(this.allColumns[0], e.value[1])
				getUserByWawo(`?wawoCode=${wawoCode}`).then(Response => {
			...
			gotoPatrolDetail() {
				...
				console.log(`${dayjs(this.finishTime).format('YYYY-MM-DD HH:mm')}:00`)
				let a = {
					remark: this.remark,
					orderType: this.order_typeDict.find(e => e.label == this.orderTypeDictName).value,
					...
					finishTime: `${dayjs(this.finishTime).format('YYYY-MM-DD HH:mm')}:00`,
