	...
	<view class="absolute">
		...
		<scroll-view scroll-y="true" class="scrollY" v-if="checkNum == 1">
			...
			<view class="reportBox">
				...
				<textarea disabled class="textareaBox" style="margin-bottom: 16rpx;" name="" id="" placeholder="请输入维修原因"
				...
				<view class="">
					<DomVideoPlayer ref="guochengdomVideo" style="height: 400rpx;"
						:src="detailInfo.videoFileList[0].url" loop controls muted
						v-if="detailInfo.videoFileList && detailInfo.videoFileList.length " />
				</view>
				<view class="reportLine">
					<view class="reportLine_L">
						<text style="color: red;">*</text>
...
<script>
	...
	export default {
		...
		methods: {
			querygetOrderList(id) {
				getOrderList(`?orderRefId=${id}`).then(res => {
					console.log(res)
					this.querygetOrderDetail(res.data.rows[0].orderId)
					this.querygetOrderProcessList(res.data.rows[0].orderId)
				})
			},
