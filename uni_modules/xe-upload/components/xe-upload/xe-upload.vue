	...
	export default {
		...
		methods: {
			...
			initInput(extension) {
				...
				this.renderInput =
					`<input type="file" id="xe-upload-${id}" name="xe-upload" ${accept ? 'accept="' + accept + '"' : ''} />`;
			},
			...
			async handleEmits(e) {
				// #ifdef APP-PLUS
				// this.$ownerInstance.callMethod('isLoading', true);
				this.isloading = true
				if (e.type === 'choose') {
					// 将base64转为本地链接
				...
				this.isloading = false
				// this.$ownerInstance.callMethod('isFloading', fa);
				// #endif
...
<script module="XeUpload" lang="renderjs">
	...
	export default {
		...
		methods: {
			...
			renderProps(info) {
				this.$ownerInstance.callMethod('isFloading', true);
				const {
					id,
					renderInput,
					upload
