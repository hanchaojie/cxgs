	...
	<view class="device-status-list">
		...
		<view class="" v-for="(item,index) in checkList" v-if="checkList" @click="gotoNext(item)">
			<view class="device-item">
				...
				<text class="device-name">{{item.partName}}
					<text :class="item.matterId?'redTxt':'blueTxt'" style="margin-left: 10rpx;font-size: 32rpx;"
						v-if="item.status !== null">{{item.matterId?`[异常]`:'[正常]'}}</text></text>
				<!-- <text></text> -->
				<image class="arrow" src="/static/arrowRed.png" v-if="item.matterId"></image>
			</view>
			<view class="infoMes" style="height: auto;">
				<!-- <image class="arrowLine" v-if="checkList.length - 1!==index" src="/static/arrow-right.png"></image> -->
				<view class="line" v-if="checkList.length - 1!==index">
				</view>
				<view class="message">
					<view class="" style="font-size: 28rpx;">
						检查时间:{{item.createTime}}
					</view>
					<view class="messageInfo" style="font-size: 28rpx;">
						检查事项:<text v-for="(i,q) in item.subjects" :key="i">{{q+1}}.{{i}}</text>
					</view>
				</view>
			</view>
